...
import { collection, doc,updateDoc, onSnapshot,arrayUnion,arrayRemove } from "firebase/firestore";
import { db } from "../../firebaseinit/firebaseinit";
// for toast notificatioins
import { toast } from "react-toastify";
const INITIAL_STATE = { productItem: [] , cart:[],
  itemInCart:0,
  myorders:[],
  total:0,
  category:"none"
...
export const getInitialCartOrdersThunk = createAsyncThunk(
    ...
    (args,thunkAPI) => {
        ...
        const {loginorlogoutReducer,productReducer} = thunkAPI.getState();
        const {isLoggedIn,userLoggedIn} = loginorlogoutReducer;
        
        // whether user logged In
        if(isLoggedIn){
            // getting real-time update of data

            const unsub = onSnapshot(doc(db, "userData",userLoggedIn.id), (doc) => {
...
const updateCartInDatabase = createAsyncThunk(
    ...
    async(args,thunkAPI) => {
        ...
        const {loginorlogoutReducer, productReducer} = thunkAPI.getState();
        const { userLoggedIn } = loginorlogoutReducer;

        // update the cart inside the firebase database
        const userRef = doc(db, "userData", userLoggedIn.id);
...
export const increaseQuantThunk = createAsyncThunk(
    ...
    async (product,thunkAPI) => {
        ...
        thunkAPI.dispatch(increaseTotalAmount(product.ProductPrice));
...
export const decreaseQuantThunk = createAsyncThunk(
    ...
    async(product,thunkAPI) => {
        ...
        thunkAPI.dispatch(reduceTotalAmount(productReducer.cart[index].ProductPrice));
...
export const getProductItems= createAsyncThunk("getproductdata",(arg,thunkAPI)=>{
  const unsub = onSnapshot(collection(db, "productData"), (Snapshot) => {
    const data = Snapshot.docs.map((doc) => ({
      id: doc.id,
      ...doc.data()
    }));
    thunkAPI.dispatch(getProduct(data));
  });
...
extraReducers: (builder) => {
    ...
    builder.addCase(getInitialCartOrdersThunk.fulfilled, (state,action) => {
        ...
        if(cart){    
            let sum=0,len =0;
            cart.map((item) => {
                Number(sum += item.ProductPrice * item.quantity);
    ...
    .addCase(removeFromCartThunk.fulfilled, (state,action) => {
        ...
        state.total -= product.quantity * product.ProductPrice;
...
export const {getProduct,setCategory,
  setMyOrders, 
...
// define and export cartSelector function here
